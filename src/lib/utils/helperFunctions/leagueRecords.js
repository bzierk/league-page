...
const processRegularSeason = async ({rosters, leagueData, curSeason, week, regularSeason}) => {
    ...
    const playerRes = await waitForAll(...playerPromises).catch((err) => {
        console.error(err);
    });
 ...
 * @param {Object[]} matchupData.playerData
 * @param {Object[]} matchupData.rosters
 * @returns {any}
 */
const processMatchups = ({
    ...
    for (const matchup of matchupWeek) {
        ...
        // Iterate through starters to find the kicker
        for (const [index, starter] of matchup.starters.entries()) {
            const starterData = playerData[starter];
            if (starterData.position === 'K') {
                // Create an object linking the rosterID to the kicker's fpts which can be used for records
                kicker_entry = {
                    rosterID,
                    kickerPoints: matchup.starters_points[index],
                    year,
                }
            }
        }
        ...
        if (kicker_entry) {
            ...
            record.updateKickerPoints(managers, kicker_entry);
        }
